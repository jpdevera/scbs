...
function loadImages() 
{
    ...
    $file_exists        = file_exists( $useruploadpath );
    ...
    if( $file_exists )
    {
        ...
        $first_count    = count( $files ) - 1;
        
        for( $i=$first_count; $i >= 0; $i-- ):
            $image                      = $files[$i];
            ...
            $image_url                  = $site.$base_url_upload_path.$image_basename;

            if(ISSET( $_SERVER['SERVER_NAME'] ) AND $_SERVER['SERVER_NAME'] == 'localhost' )
            {
               $image_url               = $site.$rootbasename.'/'.$base_url_upload_path.$image_basename;
            }
            ...
            ?>
                <div class="fileDiv"
                    ...
                    <div class="imgDiv">
                        <img class="fileImg lazy" src="<?php echo $image_url ?>" data-original="<?php echo $image_url; ?>">
                    </div>

                    <p class="fileDescription">
                        <span class="fileMime"><?php echo $image_extension; ?>
                        </span> <?php echo $image_filename; ?>
                        <?php 
                            if( $file_extens == "yes" )
                            {
                    ...
                    <p class="fileTime">
                        <?php 
                            echo date ( "F d Y H:i", filemtime( $image ) ); 
                        ?>
                    </p>
                    <p class="fileTime"><?php echo file_size_convert( filesize( $image ) ); ?></p>
                </div>
            ...
            ?>
                <div class="fullWidthFileDiv"
                    ...
                    <p class="fullWidthfileDescription">
                    ...
                    <p class="fullWidthfileTime fullWidthfileMime fullWidthlastChild">
                        <?php echo $image_extension; ?>
                    </p>
                    <p class="fullWidthfileTime">
                        <?php echo $filesizetemp; ?>
                    </p>
                    <p class="fullWidthfileTime fullWidth30percent">
                        <?php echo date ("F d Y H:i", filemtime($image)); ?>
                    </p>
                </div>
