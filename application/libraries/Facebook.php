...
class Facebook
{
    ...
    public function __construct()
    {
    	...
    	$level 			= $this->CI->config->item('rlog_level');
		$enable			= $this->CI->config->item('rlog_enable');
		$error_handler 	= $this->CI->config->item('rlog_error_handler');
        ...
        if (!ISSET($this->fb))
        {
        	 $this->fb = new FB([
                ...
                'default_graph_version' => $this->CI->config->item('facebook_graph_version')
            ]);
        }

        // Load correct helper depending on login type
        // set in the config file
        switch ($this->CI->config->item('facebook_login_type'))
        {
            case 'js':
    ...
    public function login_url()
    {
        ...
        if($this->CI->config->item('facebook_login_type') != 'web')
        {
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . $this->CI->config->item('facebook_login_redirect_url'),
    ...
    public function logout_url()
    {
        ...
        return $this->helper->getLogoutUrl(
            ...
            base_url() . $this->CI->config->item('facebook_logout_redirect_url')
        );
    }
    ...
    {
        return $this->CI->session->userdata('fb_access_token');
    }
    ...
    {
        $this->CI->session->set_userdata('fb_access_token', $access_token->getValue());
    }
    ...
    {
        return $this->CI->session->userdata('fb_expire');
    }
    ...
    private function set_expire_time(DateTime $time = null)
    {
        if ($time) 
        {
            $this->CI->session->set_userdata('fb_expire', $time->getTimestamp());
        }
    }
