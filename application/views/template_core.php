...
$header_class = ($menu_position == MENU_TOP_NAV) ? 'header-top-nav' : '';
...
<head>
	...
	<link type="text/css" href="<?php echo base_url().PATH_CSS ?>override.css" rel="stylesheet" media="all" />
...
<body class="skin_<?php echo get_setting(THEME, "skins") ?> <?php echo $header_class ?>">
	...
	<header class="cd-main-header <?php echo $class_compact_header ?> <?php echo get_setting(LAYOUT, "header") ?> ">
		...
		<div class="menu-active-opacity"></div>
		<div class="menu">
		<?php
			$active_sub_menu = ! empty($active_sub_menu) ? $active_sub_menu : '';
			get_system_modules_menu($curr_system, $active_sub_menu);
		?>
		</div>
		<?php endif; ?>
		
		<a href="#0" class="cd-nav-trigger"><span></span><div>Menu</div></a>

		<nav class="cd-nav">
			<?php 
				if($menu_position == MENU_TOP_NAV):
					get_landing_systems_menu();
				endif; 
			?>
			<ul class="cd-top-nav inline">
				<?php if($this->permission->check_permission(MODULE_ANNOUNCEMENTS)) : ?>
				<?php endif; ?>
				...
				<li class="has-children account">
					...
					<ul>
						...
						<li><a href="#modal_profile" onclick="modal_profile_init()">My Profile</a></li>
						<?php //} ?>
	...
	<div id="modal_webcam" class="modal modal-materialize modal-fixed-footer modal-fixed-header webcam-main-bg xs-w sm-h">
		<div class="modal-header">
			Take photo
			<a href="javascript:;" class="modal-action modal-close">&times;</a>
		</div>
		<form id="form_modal_webcam">
			<div class="modal-content scroll-pane scroll-dark" style="height:calc(100%-156px);">
				<div id="content"></div>
				<div class="webcam-bg">
				<!--  -->
					<div style="background : #000 !important;opacity : .5 !important;">
			...
			<div class="modal-footer right-align">
				<a href="javascript:;" class="btn-flat modal-action modal-close m-n m-r-xs"><?php echo BTN_CANCEL ?></a>
				<button type="button" id="submit_modal_webcam" class="modal-action waves-effect waves-light btn m-n" data-btn-action="<?php echo BTN_SAVING ?>"><?php echo BTN_SAVE ?></button>
			</div>
	...
	<script src="<?php echo base_url().PATH_JS ?>menu.js" type="text/javascript"></script>	
	...
	<script src="<?php echo base_url() . PATH_JS ?>eventsource.js" type="text/javascript"></script>
	...
	<?php 
		if( ISSET( $resources ) )
		{
			if( ISSET( $resources['load_materialize_modal'] ) )
			{
				$resources['load_materialize_modal']['modal_profile'] = array (
					'title' => 'Profile',
					'size' 	=> "lg full-h",
				...
				$resources['load_materialize_modal']['modal_verify_code'] = array (
					'title' => "Change Email",
					'size' => "xs-w xs-h",
					'module' => CORE_USER_MANAGEMENT,
					'controller' => "Profile_account",
					'method' => "modal_verify_code",
					// 'fixed_header' => false,
					// 'modal_footer' => false,
					// 'footer_div_none'	=> true,
					'cancel_btn_flag' => TRUE,
					'custom_button'	=> array(
						'Send Code' => array("type" => "button", "action" => 'Sending Code', 'class' => 'send_code_btn'),
						'Resend Code' => array("type" => "button", "action" => 'Resending Code', 'class' => 'resend_btn'),
						'Verify' => array("type" => "button", "action" => 'Verifying', 'class' => 'verify_btn')
					),
					'post'			=> true
				);
			}
