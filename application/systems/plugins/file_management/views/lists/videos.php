			...
			<ul class='list-action-buttons circles p-n' >
				<?php 
					if( $view_per AND $show_view ) :
						...
						$pass_args_json 	= json_encode( $pass_args );

						// if( $file_size_num > 1000000 ) :
				?>
					<li ><a data-target="modal_video_player" class="modal_video_player_trigger btn-floating btn-medium waves-effect waves-light tooltipped" data-tooltip='View Video' onclick="modal_video_player_init('', this, 'Video Player')" data-modal_post='<?php echo $pass_args_json ?>' ><i class="material-icons">open_in_new</i></a>
					<?php
						// else :
					?>
					...
					<!-- <li ><a class="btn-floating btn-medium waves-effect waves-light tooltipped" data-file-name="<?php //echo $file_name ?>" data-file-ext="<?php //echo $ext ?>" target="_blank" data-tooltip='View PDF' href="<?php echo $force_url ?>"><i class="material-icons">open_in_new</i></a> -->
					</li>
					<?php
						// endif;
					?>
