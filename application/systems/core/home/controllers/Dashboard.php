...
class Dashboard extends SYSAD_Controller
{
	...
	public function __construct()
	{
		...
		$this->module_js 	= HMVC_FOLDER . "/" . SYSTEM_CORE . "/" . CORE_DASHBOARD . "/sign_up";
		...
		try {
			...
			$resources['load_materialize_modal'] = array(
				'modal_user_details' => array(
					...
					'modal_style'	=> 'modal-closed-question'
				)
			);
			...
			$data['new_users'] = array_map(function($user) {
				$user['photo'] = $this->_construct_avatar($user);
				return $user;
			}, $this->dshm->get_new_users());
	...
	public function modal()
	{
		...
		try {
			...
			$data['params'] 				= $params;
			$data['status']					= $this->dshm->get_status();
	...
	private function _validate_update_user(&$params, $action = NULL)
	{
		...
		if (isset($params['role'])) {
			$params['role'] = array_filter(explode(',', $params['role'][0]), function($item) use($params) {
				if ($item != $params['main_role'][0])
					return $item;
			});

			if (empty($params['role'][0])) {
				unset($params['role']);
			}
		}
	...
	public function delete_incomplete_user()
	{
		...
		try
		{
			...
			if( !EMPTY( $main_where ) )
			{
				...
				$org_codes 		= array_column($prev_us, 'org_code');
